...
- (void)viewDidLoad {
    ...
    //로그인 버튼
    self.btn1 = [[UIButton alloc]initWithFrame:CGRectMake(50, 130, 100, 40)];
    [self.btn1 setTitle:@"로그인" forState: UIControlStateNormal];
    [self.btn1 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
    [center addSubview:self.btn1];
    
    //회원가입 버튼
    self.btn2 = [[UIButton alloc]initWithFrame:CGRectMake(170, 130, 100, 40)];
    [self.btn2 setTitle:@"회원가입" forState: UIControlStateNormal];
    [self.btn2 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    [self.btn2 addTarget:self action:@selector(touchupInsideJoin:) forControlEvents:UIControlEventTouchUpInside];
    [center addSubview:self.btn2];
...
- (void)touchupInsideJoin:(UIButton *)button {
