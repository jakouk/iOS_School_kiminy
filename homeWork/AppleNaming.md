...
변수명 명확하게 
animals -> animalNames
table -> animalListTable
header -> animalListAllKeys
...
//변경전 메소드안에 메소드를 쓸때
[[UITableView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height) style:UITableViewStylePlain];
...
//데이터 저장
@property NSDictionary *animalNames;

@property NSMutableArray *checkONList;
@property UITableView *animalListTable;
@property BOOL isEditing;

//animals의 allkeys 저장
@property NSArray *animalListAllKeys;
...
- (void)setAnimalData {
    self.animalNames = @{@"B" : @[@"Bear", @"Black Swan", @"Buffalo"],
                     @"C" : @[@"Camel", @"Cockatoo"],
                     @"D" : @[@"Dog", @"Donkey"],
                     @"E" : @[@"Emu"],
                     @"G" : @[@"Giraffe", @"Greater Rhea"],
...
- (void)viewDidLoad {
    ...
    self.animalListTable = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height) style:UITableViewStylePlain];
    self.animalListTable.delegate = self;
    self.animalListTable.dataSource = self;
    [self.view addSubview:self.animalListTable];
...
- (void)onClickEditButton:(UIBarButtonItem *)item {
    ...
    if (!self.isEditing) {
        //테이블 뷰 에디트 모드
        [self.animalListTable setEditing:YES animated:YES];
        self.isEditing = YES;
        
    } else {
        //일반 모드
        [self.animalListTable setEditing:NO animated:NO];
        self.isEditing = NO;
    }
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    ...
    self.animalListAllKeys = self.animalNames.allKeys;
    return self.animalListAllKeys.count;
}

//row 생성
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    
    NSArray *cell = [self.animalNames objectForKey:self.animalListAllKeys[section]];
    return cell.count;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UITableViewCell *cell;
    NSArray *row = [self.animalNames objectForKey:self.animalListAllKeys[indexPath.section]];
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
    ...
    return self.animalListAllKeys[section];
}
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //데이터 지우기
    [[self.animalNames objectForKey:self.animalListAllKeys[indexPath.section]] removeObjectAtIndex:indexPath.row];
